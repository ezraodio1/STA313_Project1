...
ggplot(taylor_albums_plot2a, aes(x = album_release)) +
  ...
  geom_smooth(aes(y = attribute_value), method = "lm", se = FALSE, color = "#DEA9C9", size =2) +
  geom_point(aes(y = attribute_value, color = album_name), size = 7, shape = 18) +
  geom_label_repel(aes(y = attribute_value, label = album_name), color = "#4D4D4D", size = 3) +
  ...
  theme(
    ...
    plot.title = element_text(vjust = -7, size = 20, face = "bold")
  ) +
  facet_wrap(~song_attribute, ncol = 1)
