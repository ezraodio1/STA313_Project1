...
taylor_albums_stats <- taylor_all_songs |>
  drop_na(energy, danceability, speechiness, acousticness, liveness, valence) |>
  group_by(album_name) |>
  summarise(
    album_energy = mean(energy),
    album_danceability = mean(danceability),
    album_speechiness = mean(speechiness),
    album_liveness = mean(liveness),
...
ggplot(taylor_albums_plot2, aes(x = album_release)) +
  ...
  geom_point(aes(y = album_liveness), color = "orange") +
  ...
  geom_line(aes(y = album_liveness), color = "orange") +
